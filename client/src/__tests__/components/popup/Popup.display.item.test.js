        ...
        rawItems: [
            {
                ...
                eventList: ['test event 1', 'test event 2']
            },
            {
                _id: 'test id 2',
                description: 'test description 2',
                expireDate: 'test expireDate 2',
...
describe('(connected) Popup -- Display Item', () => {
    const store = mockStore(displayItemPopupStore);
    ...
    it('should have correct payload as in the store', () => {
        expect(wrapper.prop('payload')).toEqual(displayItemPopupStore.popup.payload);
    });

    it('should have correct initial state', () => {
        expect(component.state('editing')).toBe(false);
        expect(component.state('payload')).toEqual(displayItemPopupStore.items.rawItems[0]);
    });
    ...
    it('should initially have exact number of Rows with correct props', () => {
        ...
        for (let field of fields) {
            expect(rows.find({
                ...
                details: displayItemPopupStore.items.rawItems[0][field]
            })).toHaveLength(1);
        }
    ...
    it('should update state properly when the name input\'s value is changed', () => {
        ...
        const expected = clonedeep(displayItemPopupStore.items.rawItems[0]);
    ...
    it('should handle submission properly when the user click the submit icon', () => {
        ...
        component.setState({ payload: {
            location: displayItemPopupStore.items.rawItems[0].location
        }});
